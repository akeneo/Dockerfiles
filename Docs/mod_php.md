...
- `pim-cac` script will completely remove the application cache, then regenerate it,
    only in prod and dev mode for the [`akeneo` image](https://github.com/damien-carcel/Dockerfiles/blob/master/akeneo/files/pim-cac.sh),
    and only in behat mode for the [`akeneo-behat` image](https://github.com/damien-carcel/Dockerfiles/blob/master/akeneo-behat/files/pim-cac.sh) image.

- `pim-assets` script will completely remove the application cache and assets (`web/bundles/*`, `web/css/*`, `web/js/*`).
    Then it will regenerate the cache and the ORO requirejs configuration, and dump all assets as symlinks.
    It will do so only in prod and dev mode for the [`akeneo` image](https://github.com/damien-carcel/Dockerfiles/blob/master/akeneo/files/pim-assets.sh),
    and only in behat mode for the [`akeneo-behat` image](https://github.com/damien-carcel/Dockerfiles/blob/master/akeneo-behat/files/pim-assets.sh) image.
...
- `pim-initialize` script will completely remove the application cache, logs, assets (`web/bundles/*`, `web/css/*`, `web/js/*`), and everything generated by any previous installation.
    Then it will regenerate the cache and the ORO requirejs configuration, perform a installation of Akeneo through the command `pim:install --force`, and dump all assets as symlinks.
    It will do so only in prod and dev mode for the [`akeneo` image](https://github.com/damien-carcel/Dockerfiles/blob/master/akeneo/files/pim-initialize.sh),
    and only in behat mode for the [`akeneo-behat` image](https://github.com/damien-carcel/Dockerfiles/blob/master/akeneo-behat/files/pim-initialize.sh) image.
